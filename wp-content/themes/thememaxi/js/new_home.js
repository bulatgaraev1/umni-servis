...
(function($) {
	$(document).ready(function() {
		$('.mob-nav--trigger').click(function(e) {
			...
			$('.header__menu.menu').slideToggle(300);
			$('body').toggleClass('mob-nav--open');
		});

		$('.header__trigger--navInfo').click(function(e) {
			e.preventDefault();
			$('.header.header--scroll').toggleClass('active');
			$('.navigationInfo').fadeToggle();
		});

		if ($(window).width() < 991) {
		   $('.header__menu.menu').wrapInner('<div class="inner"></div>');
		}
		...
		function onScroll() {
			...
			if (scroll >= 50) {
				...
				$('.navigationInfo').fadeOut();
			}
		}

		$('.seo-text__toggle').click(function(e) {
			e.preventDefault();
			$('.seo-text__text--more').toggleClass('show');
			$(this).toggleClass('active');
		});
		...
		$('#carousel-cases').owlCarousel({
		    ...
		    onInitialized: function(e) {
			    ...
			    var carousel = e.relatedTarget;
			    let s = carousel.relative(carousel.current()) + 1;
			    $('#cases2__count--start').text(s);
			    $('#cases2__count--end').text(carousel.items().length);
		    },
		...
		$('#carousel__nav--next').click(function(e) {
			e.preventDefault();
			$('#carousel-cases .owl-next').trigger('click');
		});

		$('#carousel__nav--prev').click(function(e) {
			e.preventDefault();
			$('#carousel-cases .owl-prev').trigger('click');
		});
		...
		// ---  VIDEO custom play  ---
		/*
	  jQuery('.video-play-trigger').click(function(e) {
	    e.preventDefault();
	    //let videoUrl = jQuery(this).parent().find('video source').data('src');
	    jQuery(this).parent().addClass('play');

	    //jQuery(this).parent().find('video source').attr('src', videoUrl);
	...
	resizableSwiper('(max-width: 991px)', '.sl__slider_01', {
      pagination: {
    ...
    //if ($(window).width() < 990) {
        $('.header__notice--icon').draggable({
            cancel: false,
            containment: 'parent',
            axis: 'x',
            start: function(event, ui) {
                //console.log('start')
                $('.header__notice--icon').css('animation', 'none');
            },
            drag: function(event, ui) {
                ...
                if ( $(this).position().left >= max ) {
                    ...
                    window.location.href = $('.header__notice').attr('href');
                    return false // escape here!!!
                } else {
                    let offsetPoint = ui.position.left;
                    $('.header__notice--slide').css(
                        'clip-path', 
                        'polygon(0 0, '+ (offsetPoint + 10) +'px 0%, '+ (offsetPoint + 10) +'px 100%, 0 100%)'
                    );
                    $('.header__notice').addClass('end');
                }
            },
